...
The default value is `"lease_time=3600, router=$ipv4_gateway, server_id=169.254.0.254, server_mac=$random_mac, mtu=$node_mtu"` and `server_id=$random_mac`. The `mtu` is taken from the MTU of the node's tunnel encapsulation NIC.
